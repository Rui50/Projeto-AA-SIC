<script setup>
    ...
    const exercises = ref([]) // will hve ExerciseDataResponseDTO objs
    ...
    onMounted(async () => {
        ...
        try {
            ...
            if (workoutData) {
                ...
                scheduleType.value = workoutData.scheduleType || 'freedom';

                // Populate scheduled days
                if (workoutData.scheduledDays && workoutData.scheduledDays.length > 0) {
                    weekdays.value.forEach(day => {
                        // Backend sends full day names?
                        day.selected = workoutData.scheduledDays.includes(day.full);
                    });
                }
